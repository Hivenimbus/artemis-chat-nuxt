<template>
  <div class="h-full bg-gray-50 flex flex-col">
    ...
    <!-- Indicador de status da atualização em tempo real -->
    <div class="fixed bottom-4 left-4 z-40">
      <div
        :class="[
          'flex items-center space-x-2 px-3 py-2 rounded-full shadow-lg text-xs',
      ...
      >
        ...
        <button
          ...
          :title="autoRefresh ? 'Pausar atualizações' : 'Retomar atualizações'"
        >
          <svg v-if="autoRefresh" class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
// Função para atualizar atendimentos em segundo plano
const refreshAtendimentosRealtime = async () => {
  if (!isVisible.value || !autoRefresh.value) return

  try {
    const params = new URLSearchParams()
    if (selectedCaixaEntrada.value) {
      params.append('inbox_id', selectedCaixaEntrada.value)
    }
    params.append('last_update', new Date(lastUpdate.value).toISOString())

    const response = await $fetch(`/api/atendimentos?${params.toString()}`)

    if (response?.success && response?.data?.atendimentos) {
      // Verificar se há novos atendimentos ou atualizações
      const newAtendimentos = response.data.atendimentos
      const hasChanges = JSON.stringify(newAtendimentos) !== JSON.stringify(atendimentos.value)

      if (hasChanges) {
        console.log('Atualizando atendimentos em tempo real:', {
          anterior: atendimentos.value.length,
          novo: newAtendimentos.length,
          timestamp: new Date().toISOString()
        })

        // Preservar o contato selecionado
        const selectedId = selectedContact.value?.id
        const selectedUnreadCount = selectedContact.value?.unreadCount || 0

        // Atualizar lista
        atendimentos.value = newAtendimentos

        // Manter o contato selecionado e atualizar contador
        if (selectedId) {
          const updatedContact = atendimentos.value.find(a => a.id === selectedId)
          if (updatedContact) {
            // Incrementar contador de não lidas se houver novas mensagens
            if (updatedContact.unreadCount > selectedUnreadCount) {
              console.log(`Novas mensagens para ${updatedContact.name}: ${updatedContact.unreadCount - selectedUnreadCount}`)
              // TODO: Mostrar notificação ou indicador visual
            }
          }
        }
    ...
    // Não interromper o intervalo em caso de erro
  }
}

// Observer de visibilidade da página
const handleVisibilityChange = () => {
  isVisible.value = !document.hidden
  if (isVisible.value) {
    // Se a página voltou a ficar visível, atualizar imediatamente
    refreshAtendimentosRealtime()
  }
}
...
const startRealtimeUpdates = () => {
  stopRealtimeUpdates() // Limpar intervalo existente
  realtimeInterval.value = setInterval(refreshAtendimentosRealtime, refreshInterval)
  console.log('Atualizações em tempo real iniciadas:', refreshInterval + 'ms')
}
...
onMounted(() => {
  ...
  // Iniciar atualizações após carregar dados iniciais
  nextTick(() => {
    setTimeout(() => {
      startRealtimeUpdates()
    }, 1000) // Esperar 1 segundo após carregar inicial
  })
})
...
// Watcher para parar/atualizar quando mudar de caixa de entrada
watch(selectedCaixaEntrada, () => {
  // Forçar atualização imediata quando mudar de caixa
  lastUpdate.value = Date.now() - refreshInterval // Forçar refresh
  refreshAtendimentosRealtime()
})

// Listener para eventos online/offline
const handleOnlineStatus = () => {
  if (navigator.onLine) {
    console.log('Conexão restaurada, atualizando...')
    refreshAtendimentosRealtime()
  }
}
...
defineExpose({
  toggleAutoRefresh,
  refreshAtendimentos: refreshAtendimentosRealtime,
  isAutoRefreshEnabled: () => autoRefresh.value
})
